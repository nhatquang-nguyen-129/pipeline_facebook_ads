...
if not all([
    ...
    raise EnvironmentError(
        "❌ [BACKFILL] Failed to execute Facebook Ads campaign insights backfill due to missing required environment variables."
    )
...
def backfill():
    ...
    try:
        ...
        raise ValueError(
            "❌ [BACKFILL] Failed to execute Facebook Ads campaign insights backfill due to start_date and end_date must be in YYYY-MM-DD format."
        )

    if start_date > end_date:

        raise ValueError(
            "❌ [BACKFILL] Failed to execute Facebook Ads campaign insights backfill due to start_date must be less than or equal to end_date."
        )

    print(
        "🔄 [BACKFILL] Triggering to execute Facebook Ads campaign insights backfill for "
    ...
    try:
        ...
        print(
            "✅ [BACKFILL] Successfully retrieved Facebook Ads access token from Google Secret Manager."
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "❌ [BACKFILL] Failed to execute Facebook Ads campaign insights backfill due to..."
        )
