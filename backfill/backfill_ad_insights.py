...
if not all([
    ...
    raise EnvironmentError(
        "❌ [BACKFILL] Failed to execute Facebook Ads ad insights backfill due to missing required environment variables."
    )
...
def backfill():
    ...
    try:
        ...
        raise ValueError(
            "❌ [BACKFILL] Failed to execute Facebook Ads ad insights backfill due to start_date and end_date must be in YYYY-MM-DD format."
        )

    if start_date > end_date:
        
        raise ValueError(
            "❌ [BACKFILL] Failed to execute Facebook Ads ad insights backfill due to start_date must be less than or equal to end_date."
        )

    print(
        "🔄 [BACKFILL] Triggering to execute Facebook Ads ad insights backfill for "
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "❌ [MAIN] Failed to execute Facebook Ads ad insights backfill due to..."
        )
