...
def fetch_campaign_metadata(fetch_campaign_ids: list[str]) -> pd.DataFrame:
        ...
        try:
            ...
            for fetch_campaign_id in fetch_campaign_ids:
                try:
                    ...
                    fetch_campaign_metadata = {f: campaign.get(f, None) for f in fetch_campaign_fields}
    ...
    # 1.1.9. Summarize fetch results for Facebook Ads campaign metadata
    finally:
...
def fetch_adset_metadata(fetch_adset_ids: list[str]) -> pd.DataFrame:
        ...
        try:            
            ...
            for fetch_adset_id in fetch_adset_ids:
                try:
                    ...
                    fetch_adset_metadata = {f: adset.get(f, None) for f in fetch_adset_fields}
                    fetch_adset_metadata["adset_id"] = fetch_adset_metadata.pop("id", None)
                    fetch_adset_metadata["adset_name"] = fetch_adset_metadata.pop("name", None)
                    fetch_adset_metadata["account_id"] = fetch_account_id
                    fetch_adset_metadata["account_name"] = fetch_account_name
                    fetch_adset_metadatas.append(fetch_adset_metadata)
                except Exception as e:
                    print(f"⚠️ [FETCH] Failed to retrieve Facebook Ads adset metadata for adset_id {fetch_adset_id} due to {e}.")
                    logging.error(f"⚠️ [FETCH] Failed to retrieve Facebook Ads adset metadata for adset_id {fetch_adset_id} due to {e}.")
            fetch_df_flattened = pd.DataFrame(fetch_adset_metadatas)
            if len(fetch_adset_metadatas) == len(fetch_adset_ids):
                fetch_sections_status[fetch_section_name] = "succeed"
                print(f"✅ [FETCH] Successfully retrieved Facebook Ads adset metadata with {len(fetch_adset_metadatas)}/{len(fetch_adset_ids)} adset_id(s) for account_id {fetch_account_id}.")
                logging.info(f"✅ [FETCH] Successfully retrieved Facebook Ads adset metadata with {len(fetch_adset_metadatas)}/{len(fetch_adset_ids)} adset_id(s) for account_id {fetch_account_id}.")
            elif len(fetch_adset_ids) > 0 and len(fetch_adset_metadatas) < len(fetch_adset_ids):
                fetch_sections_status[fetch_section_name] = "partial"
                print(f"⚠️ [FETCH] Partially retrieved Facebook Ads adset metadata with {len(fetch_adset_metadatas)}/{len(fetch_adset_ids)} adset_id(s) for account_id {fetch_account_id}.")
                logging.warning(f"⚠️ [FETCH] Partially retrieved Facebook Ads adset metadata with {len(fetch_adset_metadatas)}/{len(fetch_adset_ids)} adset_id(s) for account_id {fetch_account_id}.")
            else:
                fetch_sections_status[fetch_section_name] = "failed"
    ...
    # 1.2.8. Trigger to enforce schema for Facebook Ads adset metadata
        ...
        try:            
            ...
            if fetch_status_enforced == "schema_succeed_all":
                ...
                logging.info(f"✅ [FETCH] Successfully triggered Facebook Ads adset metadata schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            elif fetch_status_enforced == "schema_succeed_partial":
                fetch_sections_status[fetch_section_name] = "partial"
                print(f"⚠️ [FETCH] Partially triggered Facebook Ads adset metadata schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
                logging.warning(f"⚠️ [FETCH] Partially triggered Facebook Ads adset metadata schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            else:
                fetch_sections_status[fetch_section_name] = "failed"
    ...
    # 1.2.9. Summarize fetch results for Facebook Ads adset metadata
    finally:
        fetch_time_elapsed = round(time.time() - fetch_time_start, 2)
        fetch_df_final = fetch_df_enforced.copy() if "fetch_df_enforced" in locals() and not fetch_df_enforced.empty else pd.DataFrame()
        fetch_sections_total = len(fetch_sections_status) 
        fetch_sections_failed = [k for k, v in fetch_sections_status.items() if v == "failed"] 
        fetch_sections_succeeded = [k for k, v in fetch_sections_status.items() if v == "succeed"]
        fetch_rows_input = len(fetch_adset_ids)
        fetch_rows_output = len(fetch_df_final)
        fetch_sections_summary = list(dict.fromkeys(
            list(fetch_sections_status.keys()) +
            list(fetch_sections_time.keys())
        ...
        if fetch_sections_failed:
            ...
            logging.error(f"❌ [FETCH] Failed to complete Facebook Ads adset metadata fetching with {fetch_rows_output}/{fetch_rows_input} fetched row(s) due to {', '.join(fetch_sections_failed)} failed section(s) in {fetch_time_elapsed}s.")
        elif fetch_rows_output == fetch_rows_input:
            fetch_status_final = "fetch_succeed_all"
...
def fetch_ad_metadata(fetch_ad_ids: list[str]) -> pd.DataFrame:
    ...
    fetch_sections_status = {}
    fetch_sections_time = {}
    print(f"🔍 [FETCH] Proceeding to fetch Facebook Ads ad metadata at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    logging.info(f"🔍 [FETCH] Proceeding to fetch Facebook Ads ad metadata at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    ...
    # 1.3.5. Get Facebook Ads account_id from Google Secret Manager
        fetch_section_name = "[FETCH] Get Facebook Ads account_id from Google Secret Manager"
        fetch_section_start = time.time()
        try:
            print(f"🔍 [FETCH] Retrieving Facebook Ads ad account ID for account {ACCOUNT} from Google Secret Manager...")
        ...
        try:            
            ...
            for fetch_ad_id in fetch_ad_ids:
                try:
                    ...
                    fetch_ad_metadata = {f: ad.get(f, None) for f in fetch_ad_fields}
                    fetch_ad_metadata["ad_id"] = fetch_ad_metadata.pop("id", None)
                    fetch_ad_metadata["ad_name"] = fetch_ad_metadata.pop("name", None)
                    fetch_ad_metadata["account_id"] = fetch_account_id
                    fetch_ad_metadata["account_name"] = fetch_account_name
                    fetch_ad_metadatas.append(fetch_ad_metadata)
                except Exception as e:
                    print(f"⚠️ [FETCH] Failed to retrieve Facebook Ads ad metadata for ad_id {fetch_ad_id} due to {e}.")
                    logging.error(f"⚠️ [FETCH] Failed to retrieve Facebook Ads ad metadata for ad_id {fetch_ad_id} due to {e}.")
            fetch_df_flattened = pd.DataFrame(fetch_ad_metadatas)
            if len(fetch_ad_metadatas) == len(fetch_ad_ids):
                fetch_sections_status[fetch_section_name] = "succeed"
                print(f"✅ [FETCH] Successfully retrieved Facebook Ads ad metadata with {len(fetch_ad_metadatas)}/{len(fetch_ad_ids)} ad_id(s) for account_id {fetch_account_id}.")
                logging.info(f"✅ [FETCH] Successfully retrieved Facebook Ads ad metadata with {len(fetch_ad_metadatas)}/{len(fetch_ad_ids)} ad_id(s) for account_id {fetch_account_id}.")
            elif len(fetch_ad_ids) > 0 and len(fetch_ad_metadatas) < len(fetch_ad_ids):
                fetch_sections_status[fetch_section_name] = "partial"
                print(f"⚠️ [FETCH] Partially retrieved Facebook Ads ad metadata with {len(fetch_ad_metadatas)}/{len(fetch_ad_ids)} ad_id(s) for account_id {fetch_account_id}.")
                logging.warning(f"⚠️ [FETCH] Partially retrieved Facebook Ads ad metadata with {len(fetch_ad_metadatas)}/{len(fetch_ad_ids)} ad_id(s) for account_id {fetch_account_id}.")
            else:
                fetch_sections_status[fetch_section_name] = "failed"
    ...
    # 1.3.8. Trigger to enforce schema for Facebook Ads ad metadata
        ...
        try:
            ...
            if fetch_status_enforced == "schema_succeed_all":
                ...
                logging.info(f"✅ [FETCH] Successfully triggered Facebook Ads ad metadata schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            elif fetch_status_enforced == "schema_succeed_partial":
                fetch_sections_status[fetch_section_name] = "partial"
                print(f"⚠️ [FETCH] Partially triggered Facebook Ads ad metadata schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
                logging.warning(f"⚠️ [FETCH] Partially triggered Facebook Ads ad metadata schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            else:
                fetch_sections_status[fetch_section_name] = "failed"
    ...
    # 1.3.11. Summarize fetch results for Facebook Ads ad metadata
    finally:
        fetch_time_elapsed = round(time.time() - fetch_time_start, 2)
        fetch_df_final = fetch_df_enforced.copy() if "fetch_df_enforced" in locals() and not fetch_df_enforced.empty else pd.DataFrame()
        fetch_sections_total = len(fetch_sections_status) 
...
def fetch_ad_creative(fetch_ad_ids: list[str]) -> pd.DataFrame:
    print(f"🚀 [FETCH] Starting to fetch Facebook Ads ad creative for {len(fetch_ad_ids)} ad_id(s)...")
    logging.info(f"🚀 [FETCH] Starting to fetch Facebook Ads ad creative for {len(fetch_ad_ids)} ad_id(s)...")
    ...
    fetch_sections_status = {}
    fetch_sections_time = {}
    print(f"🔍 [FETCH] Proceeding to fetch Facebook Ads ad creative at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    logging.info(f"🔍 [FETCH] Proceeding to fetch Facebook Ads ad creative at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    ...
    # 1.4.6. Make Facebook Ads API call for ad creative
        ...
        try:            
            ...
            if len(fetch_ad_creatives) == len(fetch_ad_ids):
                fetch_sections_status[fetch_section_name] = "succeed"
                print(f"✅ [FETCH] Successfully retrieved Facebook Ads ad creative with {len(fetch_ad_creatives)}/{len(fetch_ad_ids)} ad_id(s) for account_id {fetch_account_id}.")
                logging.info(f"✅ [FETCH] Successfully retrieved Facebook Ads ad creative with {len(fetch_ad_creatives)}/{len(fetch_ad_ids)} ad_id(s) for account_id {fetch_account_id}.")
            elif len(fetch_ad_ids) > 0 and len(fetch_ad_creatives) < len(fetch_ad_ids):
                fetch_sections_status[fetch_section_name] = "partial"
                print(f"⚠️ [FETCH] Partially retrieved Facebook Ads ad creative with {len(fetch_ad_creatives)}/{len(fetch_ad_ids)} ad_id(s) for account_id {fetch_account_id}.")
                logging.warning(f"⚠️ [FETCH] Partially retrieved Facebook Ads ad creative with {len(fetch_ad_creatives)}/{len(fetch_ad_ids)} ad_id(s) for account_id {fetch_account_id}.")
            else:
                fetch_sections_status[fetch_section_name] = "failed"
    ...
    # 1.4.7. Trigger to enforce schema for Facebook Ads ad creative
        ...
        try:
            ...
            if fetch_status_enforced == "schema_succeed_all":
                ...
                logging.info(f"✅ [FETCH] Successfully triggered Facebook Ads ad creative schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            elif fetch_status_enforced == "schema_succeed_partial":
                fetch_sections_status[fetch_section_name] = "partial"
                print(f"⚠️ [FETCH] Partially triggered Facebook Ads ad creative schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
                logging.warning(f"⚠️ [FETCH] Partially triggered Facebook Ads ad creative schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            else:
                fetch_sections_status[fetch_section_name] = "failed"
...
def fetch_campaign_insights(fetch_date_start: str, fetch_date_end: str) -> pd.DataFrame:
    ...
    # 2.1.8. Trigger to enforce schema for Facebook Ads campaign insights
        ...
        try:            
            ...
            if fetch_status_enforced == "schema_succeed_all":
                ...
                logging.info(f"✅ [FETCH] Successfully triggered Facebook Ads campaign insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            elif fetch_status_enforced == "schema_succeed_partial":
                fetch_sections_status[fetch_section_name] = "partial"
                print(f"⚠️ [FETCH] Partially triggered Facebook Ads campaign insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
                logging.warning(f"⚠️ [FETCH] Partially triggered Facebook Ads campaign insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            else:
                fetch_sections_status[fetch_section_name] = "failed"
                print(f"❌ [FETCH] Failed to trigger Facebook Ads campaign insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
                logging.error(f"❌ [FETCH] Failed to trigger Facebook Ads campaign insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
        finally:
            fetch_sections_time[fetch_section_name] = round(time.time() - fetch_section_start, 2)

    # 2.1.9. Summarize fetch results for Facebook Ads campaign insights
    finally:
        fetch_time_elapsed = round(time.time() - fetch_time_start, 2)
        fetch_df_final = fetch_df_enforced.copy() if "fetch_df_enforced" in locals() and not fetch_df_enforced.empty else pd.DataFrame()
        fetch_sections_total = len(fetch_sections_status) 
        ...
        if fetch_sections_failed:
            ...
            logging.error(f"❌ [FETCH] Failed to complete Facebook Ads campaign insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) due to {', '.join(fetch_sections_failed)} failed section(s) in {fetch_time_elapsed}s.")
        elif fetch_days_output == fetch_days_input:
            fetch_status_final = "fetch_succeed_all" 
            print(f"🏆 [FETCH] Successfully completed Facebook Ads campaign insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) in {fetch_time_elapsed}s.")
            logging.info(f"🏆 [FETCH] Successfully completed Facebook Ads campaign insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) in {fetch_time_elapsed}s.")
        else:
            fetch_status_final = "fetch_succeed_partial"
            print(f"⚠️ [FETCH] Partially completed Facebook Ads campaign insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) in {fetch_time_elapsed}s.")
            logging.warning(f"⚠️ [FETCH] Partially completed Facebook Ads campaign insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) in {fetch_time_elapsed}s.")                    
        fetch_results_final = {
            "fetch_df_final": fetch_df_final,
            "fetch_status_final": fetch_status_final,
            "fetch_summary_final": {
...
def fetch_ad_insights(fetch_date_start: str, fetch_date_end: str) -> pd.DataFrame:
    ...
    fetch_sections_status = {}
    fetch_sections_time = {}
    print(f"🔍 [FETCH] Proceeding to fetch Facebook Ads ad insights from {fetch_date_start} to {fetch_date_end} at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    logging.info(f"🔍 [FETCH] Proceeding to fetch Facebook Ads ad insights from {fetch_date_start} to {fetch_date_end} at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    ...
    # 2.2.4. Initialize Facebook SDK session from access token
        fetch_section_name = "[FETCH] Initialize Facebook SDK session from access token"
        fetch_section_start = time.time()
        try:
            print(f"🔍 [FETCH] Initializing Facebook SDK session for account {ACCOUNT} with access token...")
    ...
    # 2.2.5. Get Facebook Ads account_id from Google Secret Manager
        fetch_section_name = "[FETCH] Get Facebook Ads account_id from Google Secret Manager"
        fetch_section_start = time.time()         
        try:
            print(f"🔍 [FETCH] Retrieving Facebook Ads ad account ID for account {ACCOUNT} from Google Secret Manager...")
    ...
    # 2.2.6. Make Facebook Ads API call for ad account information
        fetch_section_name = "[FETCH] Make Facebook Ads API call for ad account information"
        fetch_section_start = time.time()
        try: 
            print(f"🔍 [FETCH] Retrieving Facebook Ads account name for account_id {fetch_account_id}...")
    ...
    # 2.2.7. Make Facebook Ads API call for ad insights
        ...
        try:            
            ...
            for fetch_attempt_queued in range(fetch_attempts_queued) :
                try:
                    ...
                    fetch_ad_response = fetch_account_prefixed.get_insights(
                        fields=fetch_ad_fields,
                        params=fetch_ad_params
                    )
                    fetch_ad_insights = [dict(fetch_ad_insight) for fetch_ad_insight in fetch_ad_response] 
                    fetch_df_flattened = pd.DataFrame(fetch_ad_insights)
                    fetch_sections_status[fetch_section_name] = "succeed"
                    print(f"✅ [FETCH] Successfully retrieved Facebook Ads ad insights with {len(fetch_df_flattened)} row(s) from {fetch_date_start} to {fetch_date_end} with attempt {fetch_attempt_queued + 1}/{fetch_attempts_queued}.")
                    logging.info(f"✅ [FETCH] Successfully retrieved Facebook Ads ad insights with {len(fetch_df_flattened)} row(s) from {fetch_date_start} to {fetch_date_end} with attempt {fetch_attempt_queued + 1}/{fetch_attempts_queued}.")
                    break
                except Exception as e:
    ...
    # 2.2.8. Trigger to enforce schema for Facebook Ads ad insights
        ...
        try:            
            ...
            if fetch_status_enforced == "schema_succeed_all":
                ...
                logging.info(f"✅ [FETCH] Successfully triggered Facebook Ads ad insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            elif fetch_status_enforced == "schema_succeed_partial":
                fetch_sections_status[fetch_section_name] = "partial"
                print(f"⚠️ [FETCH] Partially triggered Facebook Ads ad insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
                logging.warning(f"⚠️ [FETCH] Partially triggered Facebook Ads ad insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
            else:
                fetch_sections_status[fetch_section_name] = "failed"
                print(f"❌ [FETCH] Failed to trigger Facebook Ads ad insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
                logging.error(f"❌ [FETCH] Failed to trigger Facebook Ads ad insights schema enforcement with {fetch_summary_enforced['schema_rows_output']}/{fetch_summary_enforced['schema_rows_input']} enforced row(s) in {fetch_summary_enforced['schema_time_elapsed']}s.")
        finally:
            fetch_sections_time[fetch_section_name] = round(time.time() - fetch_section_start, 2)
        
    # 2.2.9. Summarize fetch results for Facebook Ads ad insights
    finally:
        fetch_time_elapsed = round(time.time() - fetch_time_start, 2)
        fetch_df_final = fetch_df_enforced.copy() if "fetch_df_enforced" in locals() and not fetch_df_enforced.empty else pd.DataFrame()
        fetch_sections_total = len(fetch_sections_status) 
        ...
        if fetch_sections_failed:
            ...
            logging.error(f"❌ [FETCH] Failed to complete Facebook Ads ad insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) due to {', '.join(fetch_sections_failed)} failed section(s) in {fetch_time_elapsed}s.")
        elif fetch_days_output == fetch_days_input:
            fetch_status_final = "fetch_succeed_all" 
            print(f"🏆 [FETCH] Successfully completed Facebook Ads ad insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) in {fetch_time_elapsed}s.")
            logging.info(f"🏆 [FETCH] Successfully completed Facebook Ads ad insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) in {fetch_time_elapsed}s.")
        else:
            fetch_status_final = "fetch_succeed_partial"
            print(f"⚠️ [FETCH] Partially completed Facebook Ads ad insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) in {fetch_time_elapsed}s.")
            logging.warning(f"⚠️ [FETCH] Partially completed Facebook Ads ad insights fetching from {fetch_date_start} to {fetch_date_end} with {fetch_days_output}/{fetch_days_input} fetched day(s) and {fetch_rows_output} fetched row(s) in {fetch_time_elapsed}s.")                    
        fetch_results_final = {
            "fetch_df_final": fetch_df_final,
            "fetch_status_final": fetch_status_final,
            "fetch_summary_final": {
