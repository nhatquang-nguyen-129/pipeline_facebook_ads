...
# 1.2. Build materialized table for Facebook supplier campaign performance by union all staging table(s)
def mart_campaign_supplier() -> dict:
    ...
    # 1.2.1. Start timing the Facebook Ads supplier campaign performance materialization
    mart_time_start = time.time()
    mart_sections_status = {}
    mart_sections_time = {}
    mart_section_name = "[MART] Start timing the Facebook Ads supplier campaign performance materialization"
    mart_sections_status[mart_section_name] = "succeed"
    mart_sections_time[mart_section_name] = 0.0  # just marker not real time
    ...
    # 1.2.2. Prepare table_id for Facebook Ads supplier campaign performance materialization
        mart_section_name = "[MART] Prepare table_id for Facebook Ads supplier campaign performance materialization"
        mart_section_start = time.time()    
        staging_dataset = f"{COMPANY}_dataset_{PLATFORM}_api_staging"
        staging_table_campaign = f"{PROJECT}.{staging_dataset}.{COMPANY}_table_{PLATFORM}_all_all_campaign_insights"
        mart_dataset = f"{COMPANY}_dataset_{PLATFORM}_api_mart"
    ...
    # 1.2.3. Initialize Google Secret Manager client
        ...
        try:
            ...
            print(f"✅ [MART] Successfully initialized Google Secret Manager client for Google Cloud Platform project {PROJECT}.")
            logging.info(f"✅ [MART] Successfully initialized Google Secret Manager client for Google Cloud Platform project {PROJECT}.")
            mart_sections_status[mart_section_name] = "succeed"
        except Exception as e:
            mart_sections_status[mart_section_name] = "failed"
    ...
    # 1.2.4. Get Google Sheets sheet_id for supplier list from Google Secret Manager
        mart_section_name = "[MART] Get Google Sheets sheet_id for supplier list from Google Secret Manager"
        mart_section_start = time.time()    
        try:
            ...
            print(f"✅ [MART] Successfully retrieved Google Sheets sheet_id containing supplier name list for Facebook Ads campaign performance from Google Secret Manager.")
            logging.info(f"✅ [MART] Successfully retrieved Google Sheets sheet_id containing supplier name list for Facebook Ads campaign performance from Google Secret Manager.")
            mart_sections_status[mart_section_name] = "succeed"
        except Exception as e:
            mart_sections_status[mart_section_name] = "failed"
            print(f"❌ [MART] Failed to retrieve Google Sheets sheet_id containing supplier name list for Facebook Ads campaign performance due to {e}.")
            logging.error(f"❌ [MART] Failed to retrieve Google Sheets sheet_id containing supplier name list for Facebook Ads campaign performance due to {e}.")
        finally:
            mart_sections_time[mart_section_name] = round(time.time() - mart_section_start, 2)

    # 1.2.5. Initialize Google Sheets client
        mart_section_name = "[MART] Initialize Google Sheets client"
        mart_section_start = time.time()    
        try:
    ...
    # 1.2.8. Query supplier metadata for Facebook Ads campaign performance
        mart_section_name = "[MART] Query supplier metadata for Facebook Ads campaign performance"
        mart_section_start = time.time()    
        try: 
    ...
    # 1.2.9. Query all staging table(s) for Facebook Ads supplier campaign performance
        mart_section_name = "[MART] Query all staging table(s) for Facebook Ads supplier campaign performance"
        mart_section_start = time.time()        
        try: 
            query = f"""
            ...
            logging.info(f"✅ [MART] Successfully created or replaced materialized table {mart_table_supplier} with {row_count} row(s) for Facebook Ads supplier campaign performance.")
            mart_sections_status[mart_section_name] = "succeed"
        except Exception as e:
            mart_sections_status[mart_section_name] = "failed" 
    ...
    # 1.2.10. Delete temporary supplier metadata table
        mart_section_name = "[MART] Delete temporary supplier metadata table"
        mart_section_start = time.time()         
        try:
    ...
    # 1.2.11. Summarize materialization result(s) for Facebook Ads supplier campaign performance
        ...
        if len(mart_sections_failed) > 0:
            ...
            logging.error(f"❌ [MART] Failed to complete Facebook Ads supplier campaign performance materialization due to unsuccessful section(s) {', '.join(mart_sections_failed)}.")
            mart_status_final = "mart_failed_all"
        else:
...
def mart_creative_supplier() -> dict:
    ...
    # 2.2.1. Start timing the Facebook Ads campaign performance materialization
    mart_time_start = time.time()
    mart_sections_status = {}
    mart_sections_time = {}
    ...
    # 2.2.4. Get Google Sheets sheet_id for supplier list from Google Secret Manager
        mart_section_name = "[MART] Get Google Sheets sheet_id for supplier list from Google Secret Manager"
        mart_section_start = time.time()    
        try:
            print(f"🔍 [MART] Retrieving Google Sheets sheet_id containing supplier name list for Facebook Ads creative performance from Google Secret Manager...")
            logging.info(f"🔍 [MART] Retrieving Google Sheets sheet_id containing supplier name list for Facebook Ads creative performance from Google Secret Manager...")        
            supplier_secret_id = f"{COMPANY}_secret_{DEPARTMENT}_budget_sheet_id_supplier"
            supplier_secret_name = f"projects/{PROJECT}/secrets/{supplier_secret_id}/versions/latest"
            supplier_secret_response = google_secret_client.access_secret_version(name=supplier_secret_name)
            supplier_sheet_id = supplier_secret_response.payload.data.decode("UTF-8")
            print(f"✅ [MART] Google Sheets sheet_id containing supplier name list for Facebook Ads creative performance from Google Secret Manager.")
            logging.info(f"✅ [MART] Google Sheets sheet_id containing supplier name list for Facebook Ads creative performance from Google Secret Manager.")
            mart_sections_status[mart_section_name] = "succeed"
        except Exception as e:
            mart_sections_status[mart_section_name] = "failed"
            print(f"❌ [MART] Failed to retrieve Google Sheets sheet_id containing supplier name list for Facebook Ads creative performance due to {e}.")
            logging.error(f"❌ [MART] Failed to retrieve Google Sheets sheet_id containing supplier name list for Facebook Ads creative performance due to {e}.")
        finally:
            mart_sections_time[mart_section_name] = round(time.time() - mart_section_start, 2)
    ...
    # 2.2.8. Query supplier metadata for Facebook Ads creative performance
        mart_section_name = "[MART] Query supplier metadata for Facebook Ads creative performance"
        mart_section_start = time.time()       
        try: 
            temp_table_id = f"{PROJECT}.{mart_dataset}.temp_supplier_{uuid.uuid4().hex[:8]}"
            job_config = bigquery.LoadJobConfig(write_disposition="WRITE_TRUNCATE")            
    ...
    # 2.2.9. Query staging table(s) for Facebook Ads supplier creative performance
        mart_section_name = "[MART] Query staging table(s) for Facebook Ads supplier creative performance"
        mart_section_start = time.time()
        try:
            query = f"""
                CREATE OR REPLACE TABLE `{mart_table_supplier}`
    ...
    # 2.2.10. Delete temporary supplier metadata table
        mart_section_name = "[MART] Delete temporary supplier metadata table"
        mart_section_start = time.time()    
        try:
    ...
    # 2.2.11. Summarize materialization result(s) for Facebook Ads supplier creative performance
        ...
        if len(mart_sections_failed) > 0:
            ...
            logging.error(f"❌ [MART] Failed to complete Facebook Ads supplier creative performance materialization due to unsuccessful section(s) {', '.join(mart_sections_failed)}.")
            mart_status_final = "mart_failed_all"
        else:
