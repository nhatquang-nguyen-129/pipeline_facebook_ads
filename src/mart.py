...
def mart_campaign_all() -> dict:
    ...
    mart_sections_time = {}
    print(f"🔍 [MART] Proceeding to build materialized table for Facebook Ads campaign performance at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    logging.info(f"🔍 [MART] Proceeding to build materialized table for Facebook Ads campaign performance at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    ...
    # 1.1.4. Query staging table for materialization
        ...
        try:
            ...
            mart_sections_status[mart_section_name] = "succeed"
            print(f"✅ [MART] Successfully created or replace materialized table {mart_table_all} for Facebook Ads campaign performance with {mart_rows_uploaded} row(s).")
            logging.info(f"✅ [MART] Successfully created or replace materialized table {mart_table_all} for Facebook Ads campaign performance with {mart_rows_uploaded} row(s).")            
        except Exception as e:
            mart_sections_status[mart_section_name] = "failed"
    ...
    # 1.1.5. Summarize materialization results for Facebook Ads campaign performance
        ...
        if mart_sections_failed:
            ...
            logging.error(f"❌ [MART] Failed to complete Facebook Ads campaign performance materialization with {mart_rows_output} materialized row(s) due to {', '.join(mart_sections_failed)} failed section(s) in {mart_time_elapsed}s.")
        else:
            mart_status_final = "mart_succeed_all"
            print(f"🏆 [MART] Successfully completed Facebook Ads campaign performance materialization with {mart_rows_output} materialized row(s) in {mart_time_elapsed}s.")
            logging.info(f"🏆 [MART] Successfully completed Facebook Ads campaign performance materialization with {mart_rows_output} materialized row(s) in {mart_time_elapsed}s.")
        mart_results_final = {
            "mart_df_final": None,
...
def mart_creative_all() -> dict:
    ...
    mart_sections_status = {}
    mart_sections_time = {}
    print(f"🔍 [MART] Proceeding to build materialized table for Facebook Ads creative performance at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    logging.info(f"🔍 [MART] Proceeding to build materialized table for Facebook Ads creative performance at {datetime.now(ICT).strftime("%Y-%m-%d %H:%M:%S")}...")
    try:
    ...
    # 2.1.4. Query staging table for materialization
        ...
        try:
            ...
            mart_rows_uploaded = list(query_count_result)[0]["mart_rows_count"]
            mart_sections_status[mart_section_name] = "succeed"
            print(f"✅ [MART] Successfully created or replace materialized table {mart_table_all} for Facebook Ads creative performance with {mart_rows_uploaded} row(s).")
            logging.info(f"✅ [MART] Successfully created or replace materialized table {mart_table_all} for Facebook Ads creative performance with {mart_rows_uploaded} row(s).")
        except Exception as e:
            mart_sections_status[mart_section_name] = "failed"
            print(f"❌ [MART] Failed to create or replace materialized table for Facebook Ads campaign performance due to {e}.")
            logging.error(f"❌ [MART] Failed to create or replace materialized table for Facebook Ads campaign performance due to {e}.")
    ...
    # 2.1.5. Summarize materialization results for Facebook Ads creative performance
        ...
        if mart_sections_failed:
            ...
            logging.error(f"❌ [MART] Failed to complete Facebook Ads creative performance materialization with {mart_rows_output} materialized row(s) due to {', '.join(mart_sections_failed)} failed section(s) in {mart_time_elapsed}s.")
        else:
            mart_status_final = "mart_succeed_all"
            print(f"🏆 [MART] Successfully completed Facebook Ads creative performance materialization with {mart_rows_output} materialized row(s) in {mart_time_elapsed}s.")
            logging.info(f"🏆 [MART] Successfully completed Facebook Ads creative performance materialization with {mart_rows_output} materialized row(s) in {mart_time_elapsed}s.")            
        mart_results_final = {
            "mart_df_final": None,
            "mart_status_final": mart_status_final,
            "mart_summary_final": {
