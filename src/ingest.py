...
def ingest_campaign_metadata(ingest_campaign_ids: list) -> pd.DataFrame:
    ...
    # 1.1.6. Delete existing rows or create new table if not exist
        ...
        # Execute table creation                
                try:    
                    print(f"🔍 [INGEST] Creating Facebook Ads campaign metadata table defined name {raw_table_campaign} with partition on {table_partition_config} and cluster on {table_clusters_config}...")
                    logging.info(f"🔍 [INGEST] Creating Facebook Ads campaign metadata table defined name {raw_table_campaign} with partition on {table_partition_config} and cluster on {table_clusters_config}...")
                    ...
                    print(f"✅ [INGEST] Successfully created Facebook Ads campaign metadata table actual name {create_table_id} with partition on {table_partition_config} and cluster on {table_clusters_config}.")
                    logging.info(f"✅ [INGEST] Successfully created Facebook Ads campaign metadata table actual name {create_table_id} with partition on {table_partition_config} and cluster on {table_clusters_config}.")
                except Exception as e:
...
def ingest_adset_metadata(ingest_adset_ids: list) -> pd.DataFrame:
    ...
    # 1.2.6. Delete existing rows or create new table if not exist
        ingest_section_name = "[INGEST] Delete existing rows or create new table if not exist"
        ingest_section_start = time.time()
        ...
        # Execute table creation                
                try:    
                    print(f"🔍 [INGEST] Creating Facebook Ads adset metadata table defined name {raw_table_adset} with partition on {table_partition_config} and cluster on {table_clusters_config}...")
                    logging.info(f"🔍 [INGEST] Creating Facebook Ads adset metadata table defined name {raw_table_adset} with partition on {table_partition_config} and cluster on {table_clusters_config}...")
                    create_table_config = bigquery.Table(
                        raw_table_adset,
                        schema=table_schemas_config
                    )
                    ...
                    create_table_execute = google_bigquery_client.create_table(create_table_config)
                    create_table_id = create_table_execute.full_table_id
                    print(f"✅ [INGEST] Successfully created Facebook Ads adset metadata table actual name {create_table_id} with partition on {table_partition_config} and cluster on {table_clusters_config}.")
                    logging.info(f"✅ [INGEST] Successfully created Facebook Ads adset metadata table actual name {create_table_id} with partition on {table_partition_config} and cluster on {table_clusters_config}.")
                except Exception as e:
        ...
        # Execute batch deletion
                try:
                    ...
                    load_table_id = f"{load_table_execute.destination.project}.{load_table_execute.destination.dataset_id}.{load_table_execute.destination.table_id}"
                    load_table_rows = google_bigquery_client.get_table(load_table_id).num_rows
                    print(f"✅ [INGEST] Successfully created temporary Facebook Ads adset metadata table {load_table_id} with {load_table_rows} row(s) for batch deletion.")
                    logging.info(f"✅ [INGEST] Successfully created temporary Facebook Ads adset metadata table {load_table_id} with {load_table_rows} row(s) for batch deletion.")
                except Exception as e:
                ...
                try:                        
                    print(f"🔍 [INGEST] Deleting {len(unique_keys_value)} existing rows of Facebook Ads adset metadata using batch deletion with unique key(s) {unique_keys_defined}...")
                    logging.info(f"🔍 [INGEST] Deleting {len(unique_keys_value)} existing rows of Facebook Ads adset metadata using batch deletion with unique key(s) {unique_keys_defined}...")
                    query_delete_condition = " AND ".join([
                        f"CAST(main.{col} AS STRING) = CAST(temp.{col} AS STRING)"
                        for col in unique_keys_config
                    ])
...
def ingest_ad_metadata(ingest_ad_ids: list) -> pd.DataFrame:
    ...
    # 1.3.6. Delete existing rows or create new table if it not exist
        ...
        # Execute table creation                
                try:    
                    print(f"🔍 [INGEST] Creating Facebook Ads ad metadata table defined name {raw_table_ad} with partition on {table_partition_effective} and cluster on {table_clusters_effective}...")
                    logging.info(f"🔍 [INGEST] Creating Facebook Ads ad metadata table defined name {raw_table_ad} with partition on {table_partition_effective} and cluster on {table_clusters_effective}...")
                    create_table_config = bigquery.Table(
                        raw_table_ad,
                    ...
                    create_table_execute = google_bigquery_client.create_table(create_table_config)
                    create_table_id = create_table_execute.full_table_id
                    print(f"✅ [INGEST] Successfully created Facebook Ads ad metadata table actual name {create_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_effective}.")
                    logging.info(f"✅ [INGEST] Successfully created Facebook Ads ad metadata table actual name {create_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_effective}.")
                except Exception as e:
        ...
        # Execute batch deletion
                try:
                    ...
                    load_table_id = f"{load_table_execute.destination.project}.{load_table_execute.destination.dataset_id}.{load_table_execute.destination.table_id}"
                    load_table_rows = google_bigquery_client.get_table(load_table_id).num_rows
                    print(f"✅ [INGEST] Successfully created temporary Facebook Ads ad metadata table {load_table_id} with {load_table_rows} row(s) for batch deletion.")
                    logging.info(f"✅ [INGEST] Successfully created temporary Facebook Ads ad metadata table {load_table_id} with {load_table_rows} row(s) for batch deletion.")
                except Exception as e:
                ...
                try:                        
                    print(f"🔍 [INGEST] Deleting {len(unique_keys_value)} existing rows of Facebook Ads ad metadata using batch deletion with unique key(s) {unique_keys_defined}...")
                    logging.info(f"🔍 [INGEST] Deleting {len(unique_keys_value)} existing rows of Facebook Ads ad metadata using batch deletion with unique key(s) {unique_keys_defined}...")
                    query_delete_condition = " AND ".join([
                        f"CAST(main.{col} AS STRING) = CAST(temp.{col} AS STRING)"
                        for col in unique_keys_config
                    ])
...
def ingest_ad_creative(ingest_ad_ids: list) -> pd.DataFrame:
    ...
    # 1.4.6. Delete existing rows or create new table if it not exist
        ingest_section_name = "[INGEST] Delete existing rows or create new table if it not exist"
        ingest_section_start = time.time()
        ...
        # Execute table creation                
                try:    
                    print(f"🔍 [INGEST] Creating Facebook Ads ad creative table defined name {raw_table_creative} with partition on {table_partition_effective} and cluster on {table_clusters_effective}...")
                    logging.info(f"🔍 [INGEST] Creating Facebook Ads ad creative table defined name {raw_table_creative} with partition on {table_partition_effective} and cluster on {table_clusters_effective}...")
                    create_table_config = bigquery.Table(
                        raw_table_creative,
                        schema=table_schemas_effective
                    )
                    ...
                    create_table_execute = google_bigquery_client.create_table(create_table_config)
                    create_table_id = create_table_execute.full_table_id
                    print(f"✅ [INGEST] Successfully created Facebook Ads ad creative table actual name {create_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_effective}.")
                    logging.info(f"✅ [INGEST] Successfully created Facebook Ads ad creative table actual name {create_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_effective}.")
                except Exception as e:
        ...
        # Execute batch deletion
                try:
                    ...
                    load_table_id = f"{load_table_execute.destination.project}.{load_table_execute.destination.dataset_id}.{load_table_execute.destination.table_id}"
                    load_table_rows = google_bigquery_client.get_table(load_table_id).num_rows
                    print(f"✅ [INGEST] Successfully created temporary Facebook Ads ad creative table {load_table_id} with {load_table_rows} row(s) for batch deletion.")
                    logging.info(f"✅ [INGEST] Successfully created temporary Facebook Ads ad creative table {load_table_id} with {load_table_rows} row(s) for batch deletion.")
                except Exception as e:
                ...
                try:                        
                    print(f"🔍 [INGEST] Deleting {len(unique_keys_value)} existing rows of Facebook Ads ad creative using batch deletion with unique key(s) {unique_keys_defined}...")
                    logging.info(f"🔍 [INGEST] Deleting {len(unique_keys_value)} existing rows of Facebook Ads ad creative using batch deletion with unique key(s) {unique_keys_defined}...")
                    query_delete_condition = " AND ".join([
                        f"CAST(main.{col} AS STRING) = CAST(temp.{col} AS STRING)"
                        for col in unique_keys_config
                    ])
...
def ingest_campaign_insights(
    ...
    # 2.1.6. Delete existing rows or create new table if not exist
            ...
            # Execute table creation                
                    try:    
                        print(f"🔍 [INGEST] Creating Facebook Ads campaign insights table defined name {raw_table_campaign} with partition on {table_partition_config} and cluster on {table_clusters_config}...")
                        logging.info(f"🔍 [INGEST] Creating Facebook Ads campaign insights table defined name {raw_table_campaign} with partition on {table_partition_config} and cluster on {table_clusters_config}...")
                        create_table_config = bigquery.Table(
                            raw_table_campaign,
                            schema=table_schemas_config
                        )
                        ...
                        create_table_execute = google_bigquery_client.create_table(create_table_config)
                        create_table_id = create_table_execute.full_table_id
                        print(f"✅ [INGEST] Successfully created Facebook Ads campaign insights table actual name {create_table_id} with partition on {table_partition_config} and cluster on {table_clusters_config}.")
                        logging.info(f"✅ [INGEST] Successfully created Facebook Ads campaign insights table actual name {create_table_id} with partition on {table_partition_config} and cluster on {table_clusters_config}.")
                    except Exception as e:
                    ...
                    print(f"🔄 [INGEST] Found Facebook Ads campaign insights table {raw_table_campaign} then existing rows deletion will be proceeding...")
            ...
            # Execute overlapping deletion
                    try: 
                        print(f"🔍 [INGEST] Validating overlapping dates in Facebook Ads campaign insights table {raw_table_campaign}...")
                        logging.info(f"🔍 [INGEST] Validating overlapping dates in Facebook Ads campaign insights table {raw_table_campaign}...")
                        ...
                        print(f"❌ [INGEST] Failed to validate overlapping dates of Facebook Ads campaign insights table {raw_table_campaign} due to {e}.")
                        logging.error(f"❌ [INGEST] Failed to validate overlapping dates of Facebook Ads campaign insights table {raw_table_campaign} due to {e}.")                    
            ...
            # Execution for table delete query
                            try:
                                print(f"🔍 [INGEST] Deleting existing rows of Facebook Ads campaign insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_campaign}...")
                                logging.info(f"🔍 [INGEST] Deleting existing rows of Facebook Ads campaign insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_campaign}...")
                                query_delete_config = f"""
                                ...
                                print(f"❌ [INGEST] Failed to delete existing rows of Facebook Ads campaign insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_campaign} due to {e}.")
                                logging.error(f"❌ [INGEST] Failed to delete existing rows of Facebook Ads campaign insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_campaign} due to {e}.")
                    else:
                ...
                ingest_sections_status[ingest_section_name] = "failed"
                print(f"❌ [INGEST] Failed to delete existing rows or create new table {raw_table_campaign} if it not exist for Facebook Ads campaign insights due to {e}.")
...
def ingest_ad_insights(
    ...
    # 2.2.6. Delete existing rows or create new table if not exist
            ingest_section_name = "[INGEST] Delete existing rows or create new table if not exist"
            ingest_section_start = time.time()
            ...
            # Execute table creation                
                    try:    
                        print(f"🔍 [INGEST] Creating Facebook Ads ad insights table defined name {raw_table_ad} with partition on {table_partition_config} and cluster on {table_clusters_config}...")
                        logging.info(f"🔍 [INGEST] Creating Facebook Ads ad insights table defined name {raw_table_ad} with partition on {table_partition_config} and cluster on {table_clusters_config}...")
                        create_table_config = bigquery.Table(
                            raw_table_ad,
                            schema=table_schemas_config
                        )
                        ...
                        create_table_execute = google_bigquery_client.create_table(create_table_config)
                        create_table_id = create_table_execute.full_table_id
                        print(f"✅ [INGEST] Successfully created Facebook Ads ad insights table actual name {create_table_id} with partition on {table_partition_config} and cluster on {table_clusters_config}.")
                        logging.info(f"✅ [INGEST] Successfully created Facebook Ads ad insights table actual name {create_table_id} with partition on {table_partition_config} and cluster on {table_clusters_config}.")
                    except Exception as e:
                    ...
                    print(f"🔄 [INGEST] Found Facebook Ads ad insights table {raw_table_ad} then existing rows deletion will be proceeding...")
            ...
            # Execute overlapping deletion
                    try: 
                        print(f"🔍 [INGEST] Validating overlapping dates in Facebook Ads ad insights table {raw_table_ad}...")
                        logging.info(f"🔍 [INGEST] Validating overlapping dates in Facebook Ads ad insights table {raw_table_ad}...")
                        query_select_config = f"""
                            SELECT DISTINCT {", ".join(unique_keys_config)}
                            FROM `{raw_table_ad}`
                        """                        
                        ...
                        logging.info(f"✅ [INGEST] Successfully validated {len(ingest_dates_overlapped)} overlapping date(s) in Facebook Ads ad insights {raw_table_ad} table.")
                    except Exception as e:
                        print(f"❌ [INGEST] Failed to validate overlapping dates of Facebook Ads ad insights table {raw_table_ad} due to {e}.")
                        logging.error(f"❌ [INGEST] Failed to validate overlapping dates of Facebook Ads ad insights table {raw_table_ad} due to {e}.")
            ...
            # Execution for table delete query
                            try:
                                ...
                                print(f"❌ [INGEST] Failed to delete existing rows of Facebook Ads ad insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_ad} due to {e}.")
                                logging.error(f"❌ [INGEST] Failed to delete existing rows of Facebook Ads ad insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_ad} due to {e}.")
                    else:
                        print(f"⚠️ [INGEST] No overlapping date of Facebook Ads ad insights found in Google BigQuery {raw_table_ad} table then deletion is skipped.")
                        logging.info(f"⚠️ [INGEST] No overlapping date of Facebook Ads ad insights found in Google BigQuery {raw_table_ad} table then deletion is skipped.")
                ingest_sections_status[ingest_section_name] = "succeed"
            
            except Exception as e:
                ingest_sections_status[ingest_section_name] = "failed"
                print(f"❌ [INGEST] Failed to delete existing rows or create new table {raw_table_ad} if it not exist for Facebook Ads ad insights due to {e}.")
                logging.error(f"❌ [INGEST] Failed to delete existing rows or create new table {raw_table_ad} if it not exist for Facebook Ads ad insights due to {e}.")
    ...
    # 2.2.7. Upload Facebook Ads ad insights to Google BigQuery
            ...
            try:
                print(f"🔍 [INGEST] Uploading {len(ingest_df_deduplicated)} deduplicated row(s) of Facebook Ads ad insights to Google BigQuery table {raw_table_ad}...")
                logging.info(f"🔍 [INGEST] Uploading {len(ingest_df_deduplicated)} deduplicated row(s) of Facebook Ads ad insights to Google BigQuery table {raw_table_ad}...")
                load_table_config = bigquery.LoadJobConfig(write_disposition="WRITE_APPEND")
                load_table_execute = google_bigquery_client.load_table_from_dataframe(
                    ingest_df_deduplicated,
                    raw_table_ad,
