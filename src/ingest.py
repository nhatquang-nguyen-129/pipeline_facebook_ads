...
def ingest_adset_metadata(adset_id_list: list) -> pd.DataFrame:
    ...
    # 1.2.9. Summarize ingestion result(s)
        ...
        if ingest_sections_failed:
            ...
            print(f"⚠️ [INGEST] Partially completed Facebook Ads adset metadata ingestion with {ingest_rows_output}/{ingest_rows_input} campaign_id(s) uploaded in {ingest_time_elapsed}s.")
            logging.warning(f"⚠️ [INGEST] Partially completed Facebook Ads adset metadata ingestion with {ingest_rows_output}/{ingest_rows_input} campaign_id(s) uploaded in {ingest_time_elapsed}s.")
            ingest_status_final = "ingest_success_partial"
        else:
            print(f"🏆 [INGEST] Successfully completed Facebook Ads adset metadata ingestion for {ingest_sections_total} section(s) with {ingest_rows_output}/{ingest_rows_input} campaign_id(s) in {ingest_time_elapsed}s.")
            logging.info(f"🏆 [INGEST] Successfully completed Facebook Ads adset metadata ingestion for {ingest_sections_total} section(s) with {ingest_rows_output}/{ingest_rows_input} campaign_id(s) in {ingest_time_elapsed}s.")
            ingest_status_final = "ingest_success_all"
        return {
            "ingest_df_final": ingest_df_final,
            "ingest_status_final": ingest_status_final,
