...
def ingest_campaign_metadata(ingest_ids_campaign: list) -> pd.DataFrame:
    ...
    # 1.1.3. Trigger to fetch Facebook Ads campaign metadata
        ...
        ingest_section_start = time.time()
        try:
            print(f"🔁 [INGEST] Triggering to fetch Facebook Ads campaign metadata for {len(ingest_ids_campaign)} campaign_id(s)...")
            logging.info(f"🔁 [INGEST] Triggering to fetch Facebook Ads campaign metadata for {len(ingest_ids_campaign)} campaign_id(s)...")
    ...
    # 1.1.4. Prepare Google BigQuery table_id for ingestion
        ...
        try:
            ...
            ingest_sections_status[ingest_section_name] = "succeed"   
            print(f"🔍 [INGEST] Preparing to ingest Facebook campaign metadata for {len(ingest_df_fetched)} fetched row(s) to Google BigQuery table_id {raw_table_campaign}...")
            logging.info(f"🔍 [INGEST] Preparing to ingest Facebook campaign metadata for {len(ingest_df_fetched)} fetched row(s) to Google BigQuery table_id {raw_table_campaign}...")
        finally:
            ingest_sections_time[ingest_section_name] = round(time.time() - ingest_section_start, 2)
    ...
    # 1.1.5. Trigger to enforce schema for Facebook Ads campaign metadata
        ...
        try:
            ...
            if ingest_status_enforced == "schema_succeed_all":
                ...
                ingest_sections_status[ingest_section_name] = "failed"
                print(f"❌ [INGEST] Failed to trigger Facebook Ads campaign metadata schema enforcement with {ingest_summary_enforced['schema_rows_output']}/{len(ingest_df_fetched)} enforced row(s) due to failed section(s) {', '.join(ingest_summary_enforced['schema_sections_failed'])} in {ingest_summary_enforced['fetch_time_elapsed']}s.")
                logging.error(f"❌ [INGEST] Failed to trigger Facebook Ads campaign metadata schema enforcement with {ingest_summary_enforced['schema_rows_output']}/{len(ingest_df_fetched)} enforced row(s) due to failed section(s) {', '.join(ingest_summary_enforced['schema_sections_failed'])} in {ingest_summary_enforced['fetch_time_elapsed']}s.")
        finally:
            ingest_sections_time[ingest_section_name] = round(time.time() - ingest_section_start, 2)
...
def ingest_ad_metadata(ingest_ids_ad: list) -> pd.DataFrame:
    print(f"🚀 [INGEST] Starting to ingest raw Facebook Ads ad metadata for {len(ingest_ids_ad)} ad_id(s)...")
    logging.info(f"🚀 [INGEST] Starting to ingest raw Facebook Ads ad metadata for {len(ingest_ids_ad)} ad_id(s)...")
    ...
    # 1.3.3. Trigger to fetch Facebook Ads ad metadata
        ...
        ingest_section_start = time.time()
        try:
            print(f"🔁 [INGEST] Triggering to fetch Facebook Ads ad metadata for {len(ingest_ids_ad)} ad_id(s)...")
            logging.info(f"🔁 [INGEST] Triggering to fetch Facebook Ads ad metadata for {len(ingest_ids_ad)} ad_id(s)...")
    ...
    # 1.3.4. Prepare Google BigQuery table_id for ingestion
        ...
        try:
            ...
            ingest_sections_status[ingest_section_name] = "succeed"
            print(f"🔍 [INGEST] Preparing to ingest Facebook Ads ad metadata for {len(ingest_ids_ad)} ad_id(s) with Google BigQuery table_id {raw_table_ad}...")
            logging.info(f"🔍 [INGEST] Preparing to ingest Facebook Ads ad metadata for {len(ingest_ids_ad)} ad_id(s) with Google BigQuery table_id {raw_table_ad}...")
        finally:
            ingest_sections_time[ingest_section_name] = round(time.time() - ingest_section_start, 2)        
...
def ingest_ad_creative(ingest_ids_ad: list) -> pd.DataFrame:
    ...
    # 1.4.4. Prepare Google BigQuery table_id for ingestion
        ...
        try:
            ...
            ingest_sections_status[ingest_section_name] = "succeed"
            print(f"🔍 [INGEST] Preparing to ingest Facebook Ads ad creative for {len(ingest_ids_ad)} ad_id(s) with Google BigQuery table_id {raw_table_creative}...")
            logging.info(f"🔍 [INGEST] Preparing to ingest Facebook Ads ad creative for {len(ingest_ids_ad)} ad_id(s) with Google BigQuery table_id {raw_table_creative}...")
        finally:
            ingest_sections_time[ingest_section_name] = round(time.time() - ingest_section_start, 2)
...
def ingest_campaign_insights(
    ...
    # 2.1.4. Trigger to enrich Facebook Ads campaign insights
            ingest_section_name = "[INGEST] Trigger to enrich Facebook Ads campaign insights"
            ingest_section_start = time.time()
            try:
    ...
    # 2.1.5. Trigger to enforce schema for Facebook Ads campaign insights
...
def ingest_ad_insights(
    ...
    ingest_loops_time = {
        ...
        "[INGEST] Trigger to enrich Facebook Ads ad insights": 0.0,
    ...
    # 2.2.4. Trigger to enrich Facebook Ads ad insights
            ingest_section_name = "[INGEST] Trigger to enrich Facebook Ads ad insights"
            ingest_section_start = time.time()
            try:
                ...
                if ingest_status_enriched == "enrich_succeed_all":
                    ...
                    ingest_sections_status[ingest_section_name] = "failed"
                    print(f"❌ [INGEST] Failed to trigger Facebook Ads ad insights enrichment for {ingest_date_separated} with {ingest_summary_enriched['enrich_rows_output']}/{ingest_summary_enriched['enrich_rows_input']} enriched row(s) due to section(s) {', '.join(ingest_summary_enriched["enrich_sections_failed"]) if ingest_summary_enriched["enrich_sections_failed"] else 'unknown error'} in {ingest_summary_enriched['enrich_time_elapsed']}s.")
                    logging.error(f"❌ [INGEST] Failed to trigger Facebook Ads ad insights enrichment for {ingest_date_separated} with {ingest_summary_enriched['enrich_rows_output']}/{ingest_summary_enriched['enrich_rows_input']} enriched row(s) due to section(s) {', '.join(ingest_summary_enriched["enrich_sections_failed"]) if ingest_summary_enriched["enrich_sections_failed"] else 'unknown error'} in {ingest_summary_enriched['enrich_time_elapsed']}s.")
            finally:
                ingest_loops_time[ingest_section_name] += round(time.time() - ingest_section_start, 2)

    # 2.2.5. Trigger to enforce schema for Facebook Ads ad insights
    ...
    # 2.2.8. Upload Facebook Ads ad insights to Google BigQuery
    ...
    # 2.2.9. Summarize ingestion result(s) for Facebook Ads ad insights
    finally:
        ingest_time_elapsed = round(time.time() - ingest_time_start, 2)
        ingest_df_final = pd.concat(ingest_dates_uploaded or [], ignore_index=True)
        ingest_sections_total = len(ingest_sections_status)
