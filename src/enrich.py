...
def enrich_campaign_fields(enrich_df_input: pd.DataFrame, enrich_table_id: str) -> pd.DataFrame:
    ...
    # 2.1.5. Enrich other field(s) for Facebook Ads campaign insights
        ...
        try:
            ...
            logging.info(f"🔍 [ENRICH] Enriching other field(s) for staging Facebook Ads campaign insights with {len(enrich_df_campaign)} row(s)...")
            ...
            enrich_sections_status[enrich_section_name] = "failed"
            print(f"❌ [ENRICH] Failed to enrich other field(s) for staging Facebook Ads ad insights due to {e}.")
            logging.error(f"❌ [ENRICH] Failed to enrich other field(s) for staging Facebook Ads ad insights due to {e}.")
        finally:
            enrich_sections_time[enrich_section_name] = round(time.time() - enrich_section_start, 2)    
