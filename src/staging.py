...
def staging_ad_insights(updated_date: pd.DataFrame):
    ...
    try:
        ...
        logging.error(f"❌ [STAGING] Failed to prepare metadata for Facebook ad insights due to {e}.")
        return
    ...
    # 1.2.3. Query, join và enrich tất cả các bảng raw Facebook ad insights (theo tháng)
    all_dfs = []
    updated_date["date"] = pd.to_datetime(updated_date["date"])
    updated_months = updated_date["date"].dt.to_period("M").unique()
    raw_tables = [
    ...
    for raw_table, month_period in zip(raw_tables, updated_months):
        ...
        try:
            df_month = client.query(query).to_dataframe()
            if not df_month.empty:
                # 1.2.4. Enrich từng tháng sau khi query
    ...
    df_all = pd.concat(all_dfs, ignore_index=True)
    print(f"✅ [STAGING] Successfully combined & enriched {len(df_all)} row(s) from all raw Facebook ad insights table(s).")
    logging.info(f"✅ [STAGING] Successfully combined & enriched {len(df_all)} row(s) from all raw Facebook ad insights table(s).")

    # 1.2.5. Enforce schema cho staging ad insights
    try:
    ...
    try:
        ...
        load_job = client.load_table_from_dataframe(df_all, staging_ad_insights, job_config=job_config)
        load_job.result()
