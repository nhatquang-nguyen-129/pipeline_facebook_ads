...
def dags_campaign_insights(
    *,
    ...
    print(
        "🔄 [DAGS] Trigger to update Facebook Ads campaign insights for account_id "
        f"{account_id} from "
        f"{start_date} to "
        f"{end_date}..."
    )
    ...
    while dags_start_date <= dags_end_date:
        ...
        # Load
                ...
                print(
                    "⚠️ [DAGS] Failed to trigger Facebook Ads campaign insights extraction from account_id "
                    f"{account_id} for "
                    f"{dags_split_date} with "
                ...
                if not retryable:
                    ...
                    raise RuntimeError(
                        "❌ [DAGS] Failed to trigger Facebook Ads campaign insights extraction from account_id "
                        f"{account_id} for "
    ...
    for attempt in range(1, DAGS_CAMPAIGN_ATTEMPTS + 1):
        ...
        if not failed_campaign_ids:
            ...
            print(
                "✅ [DAGS] Successfully triggered to extract Facebook Ads campaign metadata from account_id "
                f"{account_id} with "
